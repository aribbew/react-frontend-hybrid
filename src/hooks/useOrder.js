...
import {
  ...
  closeOrderApi,
  getOrdersByPaymentsApi,
  getOrdersCommentsByCartApi,
} from "../api/orders";
...
export function useOrder() {
  ...
  // const getOrdersCommentsByCart = async (idComment) => {
  //   try {
  //     await getOrdersCommentsByCartApi(idComment);
  //   } catch (error) {
  //     setError(error);
  //   }
  // };
  ...
  const getOrdersByPayment = async (idPayment) => {
    try {
      return await getOrdersByPaymentsApi(idPayment);
    } catch (error) {
      setError(error);
    }
  };
